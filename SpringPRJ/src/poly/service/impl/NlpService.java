...
public class NlpService implements INlpService {
	...
	// 로그 파일 생성 및 로그 출려을 위한 log4j 프레임워크의 자바 객체
	private Logger log = Logger.getLogger(this.getClass());
	
	// NLP_DIC를 ㄱㄴㄷㄹ 순으로 나눠서 저장한 이유는 전체 약 몇 만건의 데이터를 조회하는 것보다
	// 일정한 범위를 지정하여 데이터 조회 횟수를 감소하기 위해서 나눔
	// 가나다 순으로 저장될 데이터 사전을(가~하까지)
	private Map<String, List<NlpDTO>> NLP_DIC = new HashMap<String, List<NlpDTO>>();

	/**
	 * PostContruct은 처음 한 번만 실행되도록 하는 어노테이션
	 * 
	 * 데이터 사전 변수 생성을 위한 함수 최초 스프링 호출이 발생할 때 함수가 실행 이 후 더 이상 실행을 하지 않는다.
	 * 
	 * NLP_DIC 변수에 값을 저장한
	 */
	...
	public int WordAnalysisForMind(String firstWord, String text) throws Exception {
		...
		// 데이터 사전에 존재하는 것만 분석 수정
		if (dicType.length() > 0) {
			//문장의 첫 글자를 통해 해당되는 데이터 사전 가져오기
